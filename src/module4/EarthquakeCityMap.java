...
public class EarthquakeCityMap extends PApplet {
	...
	private void addKey() {	
		...
		fill(color(255, 0, 0));
		triangle(50 - 5, 125 + 5, 50 + 5, 125 + 5, 50, 125 - 5);
	...
	private void printQuakes() 
	{
		...
		for (Marker country : countryMarkers) {
			...
			for(Marker marker : quakeMarkers) {
				...
				if (marker instanceof LandQuakeMarker) {
					if (country.getProperty("name") == marker.getProperty("country")) {
						QuakCnt ++;
					}
			  }
		 }
			if (QuakCnt > 0) {
				System.out.println(country.getProperty("name") + " : " + QuakCnt);
			}
		}
		int OceanQuakCnt = 0;
		for (Marker marker : quakeMarkers) {
		...
		System.out.println("Oceans : " + OceanQuakCnt);
	}
